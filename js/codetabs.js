...
var CodeTabs = (function() {
    ...
    var exportZip = function() {
        if (confirm('Save sketch?')) {
            ...
            var img = dataFolder.folder('img');
            for (var fn in FileManager.files) {
                images.push(fn);
                var d = FileManager.files[fn].substring(FileManager.files[fn].indexOf(',') + 1);
                img.file(fn + '.png', d, {
                    base64:true
                });
            }
    ...
    var resetTabs = function() {
        $('.tab, .menu').show();
        var tabHeight = $('.tabs').height() + 1;
        if (tabHeight > 90) tabHeight = 34; //happens with reveal.js sometimes
        $('.content').css({
            top:tabHeight + 'px'
        });
    ...
    var refreshMirrorGutters = function() {
        var fontWidth;
        if (data.fontsize) {
            fontWidth = parseInt(data.fontsize.substring(0, data.fontsize.length - 2));
        } else {
            fontWidth = '16px';
        }
